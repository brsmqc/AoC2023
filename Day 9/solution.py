...
def predict_next_value(history: list) -> int:
    if sum(history) == 0:
        return 0
    next_sequence = []
    ...
    next_value = predict_next_value(next_sequence)
    new_value = next_value + history[len(history) - 1]
    return new_value
...
if __name__ == "__main__":
    ...
    for series in eco_data:
        extrapolation = predict_next_value(series)
